...
int main()
{
  ...
  groceryitem GI[50]; //groceryitem array
  int GILS = 0; //grocerryitem logical size
  groceryitem GItemp[50]; //groceryitem array to store items being brought
  int GItempLS = 0;
  ...
  while (getline (cin, xxx))
  {
    ...
    else if (comm == "close")
    {
      ...
      if (itemp == 1)
      {
        ...
        cout << setw(40) << "The money spent on importing is: " << setw(10) << pricecharge << endl;
        cout << setw(40) << "The tax spent on importing is: " << setw(10) << taxcharge << endl;
        cout << setw(40) << "The items being sold today include: " << endl;
        cout << setw(8) << "Name" << setw(8) << "Unit" << setw(8) << "Price" << setw(8) << "Tax" << endl;
        for (int i = 0; i < GItempLS; i ++)
        {
          cout << setw(8) << GItemp[i].getName() << setw(8) << GItemp[i].getInventory() <<
          setw(8) << GItemp[i].getPrice() * GItemp[i].getInventory() << setw(8) <<
          GItemp[i].getTax() * GItemp[i].getInventory() << endl;
        }
    ...
    else if (comm == "buy")
    {
      if (itemp == 1)
      {
        ...
        dues = buyIt(myline, GI, GILS, GItemp, GItempLS);

        //import the dues
        pricedue += dues.first;
        //curtail to second decimal point
        int inttemp = round (pricedue * 0.0475 * 100);
        taxdue += inttemp / 100;
        GItempLS = dues.second;
      }
